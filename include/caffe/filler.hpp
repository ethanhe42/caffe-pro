...
class OrthogonalFiller : public Filler<Dtype> {
 public:
  ...
  virtual void Fill(Blob<Dtype>* blob) {
    ...
    Dtype std;
    if (Dtype(this->filler_param_.std() == Dtype(1))) {
      std = sqrt(Dtype(2) / n);
    }
    else{
      std = Dtype(this->filler_param_.std());
      CHECK_EQ(0, 1);
    }

    caffe_rng_gaussian<Dtype>(blob->count(), Dtype(this->filler_param_.mean()), std,        blob->mutable_cpu_data());
